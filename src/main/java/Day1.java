...
public class Day1 {
    ...
    public static void main(String[] args) {
        int calibration1 = Arrays.stream(PUZZLE_INPUT.split("\n"))
                .map(Day1::calibrationValue1)
                .mapToInt(Integer::intValue)
                .sum();
        int calibration2 = Arrays.stream(PUZZLE_INPUT.split("\n"))
                .map(Day1::calibrationValue2)
                .mapToInt(Integer::intValue)
                .sum();

        System.out.println("Calibration Value 1: " + calibration1);
        System.out.println("Calibration Value 2: " + calibration2);
    }

    private static int calibrationValue1(String line) {
        int first = SPACE_CHAR;
        int last = SPACE_CHAR;
        int front = 0;
        int back = line.length() - 1;
        while (front <= back) {
            if (first != SPACE_CHAR && last != SPACE_CHAR) {
                break;
            }
            if (first == SPACE_CHAR) {
                int intVal = Character.getNumericValue(line.charAt(front));
            ...
            if (last == SPACE_CHAR) {
                int intVal = Character.getNumericValue(line.charAt(back));
                if (intVal >= 0 && intVal <= 9) {
                    last = intVal;
                } else {
                    back -= 1;
    ...
    private static int calibrationValue2(String line) {
        List<CalibrationReading> front = new ArrayList<>();
        List<CalibrationReading> back = new ArrayList<>();
        front.add(getReadingForVal(line.indexOf("1"), line.indexOf("one"), 1, true));
        front.add(getReadingForVal(line.indexOf("2"), line.indexOf("two"), 2, true));
        front.add(getReadingForVal(line.indexOf("3"), line.indexOf("three"), 3, true));
        front.add(getReadingForVal(line.indexOf("4"), line.indexOf("four"), 4, true));
        front.add(getReadingForVal(line.indexOf("5"), line.indexOf("five"), 5, true));
        front.add(getReadingForVal(line.indexOf("6"), line.indexOf("six"), 6, true));
        front.add(getReadingForVal(line.indexOf("7"), line.indexOf("seven"), 7, true));
        front.add(getReadingForVal(line.indexOf("8"), line.indexOf("eight"), 8, true));
        front.add(getReadingForVal(line.indexOf("9"), line.indexOf("nine"), 9, true));
        back.add(getReadingForVal(line.lastIndexOf("1"), line.lastIndexOf("one"), 1, false));
        back.add(getReadingForVal(line.lastIndexOf("2"), line.lastIndexOf("two"), 2, false));
        back.add(getReadingForVal(line.lastIndexOf("3"), line.lastIndexOf("three"), 3, false));
        back.add(getReadingForVal(line.lastIndexOf("4"), line.lastIndexOf("four"), 4, false));
        back.add(getReadingForVal(line.lastIndexOf("5"), line.lastIndexOf("five"), 5, false));
        back.add(getReadingForVal(line.lastIndexOf("6"), line.lastIndexOf("six"), 6, false));
        back.add(getReadingForVal(line.lastIndexOf("7"), line.lastIndexOf("seven"), 7, false));
        back.add(getReadingForVal(line.lastIndexOf("8"), line.lastIndexOf("eight"), 8, false));
        back.add(getReadingForVal(line.lastIndexOf("9"), line.lastIndexOf("nine"), 9, false));
        int first = front.stream()
                .filter(Objects::nonNull)
                .min(Comparator.comparingInt(CalibrationReading::index)).get().val();
        int last = back.stream()
                .filter(Objects::nonNull)
                .max(Comparator.comparingInt(CalibrationReading::index)).get().val();
        return first * 10 + last;
    }

    static CalibrationReading getReadingForVal(int index1, int index2, int val, boolean min) {
        if (index1 == -1 && index2 == -1) return null;
        if (index1 == -1) return new CalibrationReading(index2, val);
        if (index2 == -1) return new CalibrationReading(index1, val);
        if (min) {
            return new CalibrationReading(Math.min(index1, index2), val);
        } else {
            return new CalibrationReading(Math.max(index1, index2), val);
        }
    }

    record CalibrationReading(int index, int val) { }

    private static final String PUZZLE_INPUT = "six1mpffbnbnnlxthree\n" +
            "4eight3one92\n" +
            "9nine2xnhvjtjlzj48\n" +
